...
`max-w-[380px]`(실제 안드로이드 기기의 논리 해상도 폭 ~360dp에 맞춘 값)로 상한을 두되 `w-full`로 좁은 화면에서 줄어들게 해 반응형을 보장한다. 원본 앱 스크린샷의 dp 수치(폰트 크기, 버튼 높이 등)를 px로 그대로 옮겨도 비율이 맞도록 이 폭 기준으로 설계한다 — 프레임을 좁게 잡고 원본 dp 값을 그대로 쓰면 텍스트/버튼이 실제보다 커 보인다. 웹 앱을 재현한 데모(TodoApp 등)는 감싸지 않는다 — 원본이 데스크톱/반응형 웹이면 넓은 카드 그대로 두는 게 자연스럽다.
